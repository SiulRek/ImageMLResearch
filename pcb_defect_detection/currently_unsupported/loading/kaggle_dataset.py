...
def get_dataframe(
    ...
    for anno in all_files:
            ...
            if "object" in elem.tag:
                ...
                for attr in list(elem):
                    ...
                    if "bndbox" in attr.tag:
                        xmin = (
                            -1
                        )  # To avoid assigment of values from the previous object.
                        ymin = -1
                        xmax = -1
                        ymax = -1
        ...
        for i in range(cnt):
            ...
            dataset["path"] += [
                path_im + tmp_path[32:]
            ]  # Adapt path to pcb-defect-detection Workspace
...
def get_tf_dataset(
    path_an=PATH_ANNOTATIONS,
    path_im=PATH_IMAGE,
    create_annotation_summary=False,
    random_seed=75,
    sample_num=-1,
):
    """
...
def get_tf_datasets_for_each_category(
    path_an=PATH_ANNOTATIONS,
    path_im=PATH_IMAGE,
    create_annotation_summary=False,
    random_seed=75,
    ...
    for category in Category:
        ...
        if category_df.empty:
            continue
        datasets[category.name] = get_tf_dataset_from_df(
            category_df, random_seed=random_seed
        )
    return datasets

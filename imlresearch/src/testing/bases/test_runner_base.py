...
class TestRunnerBase(ABC):
    ...
    def __init__(self):
        """
        Initializes the TestRunnerBase instance.

        Sets up the test file path, output directory, logger, and test suite.
        """
    ...
    def _init_logger(self):
        """
        Initializes the test result logger, creating a log file
    ...
    def _infere_test_file_path(cls):
        ...
        First, it attempts to infer the file path from the module information.
        If that fails, it tries to infer the path from the command-line
        arguments.
        ...
        Raises:
            FileNotFoundError: If the test file path cannot be inferred.
        """
    ...
    def _compute_output_dir(cls, parent_folder="tests"):
        """
        Computes the directory path for test outputs.

        This method traverses up the file hierarchy until it finds a directory
        named 'tests', then returns the path to the 'outputs' subdirectory.

        Args:
            parent_folder (str, optional): The parent folder name that
                contains the 'tests' directory. Default is "tests".
    ...
    def load_tests(self):
        """
        Abstract method to load specific tests.
